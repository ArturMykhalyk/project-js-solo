...
<p class='feed-back--name'>${o}</p>
   ...
   <p class="modal-biography__title">${e}</p>
        ...
        <ul class="modal-biography__genre-list">${f}</ul>`;i.modalArtistBiography.innerHTML=B}function I(t){i.modalArtisTAlbums.innerHTML=t.map(s=>F(s)).join("")}function F(t){const s=t.tracks.map(a=>{const e=Math.floor(a.intDuration/6e4),o=Math.floor(a.intDuration%6e4/1e3).toString().padStart(2,"0");return`
